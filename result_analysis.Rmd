...
```{r summarize model coefficients}
# Analyze model coefficient directions and significance patterns
# This produces summary counts and percentages for each covariate

# Load detailed coefficient data
coef_pvalue_df <- model_coef %>%
  dplyr::select(
    Disease, Disease.Name, Disease.Category,
    contains("_Coefficient"), contains("_P_value")
  ) %>%
  # Convert to long format for analysis
  pivot_longer(
    cols = contains("_Coefficient") | contains("_P_value"),
    names_to = c("Term", ".value"),
    names_pattern = "(.+)_(Coefficient|P_value)"
  ) %>%
  # Clean up term names
  mutate(
    Term = case_when(
      Term == "dd_age_years" ~ "Age",
      Term == "dd_weight_lbs" ~ "Weight", 
      Term == "dd_breed_pure_or_mixed" ~ "Mixed Breed",
      Term == "recode.sex_2" ~ "Female Spayed",
      Term == "recode.sex_3" ~ "Male Unneutered", 
      Term == "recode.sex_4" ~ "Male Neutered",
      TRUE ~ Term
    )
  ) %>%
  filter(!is.na(Coefficient), !is.na(P_value))

# Calculate coefficient categories and statistics
...
write.csv(overall_stats, "./results/statistics/overall_coefficient_stats.csv", row.names = FALSE)
write.csv(per_covariate_stats, "./results/statistics/per_covariate_stats.csv", row.names = FALSE)
write.csv(coefficient_stats, "./results/statistics/detailed_coefficient_stats.csv", row.names = FALSE)
...
covariate_summary <- per_covariate_stats %>%
  dplyr::select(Term, total_significant, total_significant_percent, 
         positive_significant, negative_significant) %>%
  arrange(desc(total_significant_percent))

cat("\nCovariate significance summary (ordered by % significant):\n")
