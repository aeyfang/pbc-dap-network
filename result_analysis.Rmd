...
reference <- read.csv("./frequency tables/disease_frequencies_unstrat.csv") %>%
  select(Code.from.DAP.data, Disease.Name) %>%
  mutate(Code.from.DAP.data = as.character(Code.from.DAP.data))

# Map function with fallback
map_disease_name <- function(code) {
  clean_code <- sub("^condition_", "", code)
  ref <- reference$Disease.Name[reference$Code.from.DAP.data == clean_code]
  if (length(ref) == 1) return(ref)
  return(code)
}

# Get numeric part for ordering
get_code_number <- function(code) {
  as.numeric(sub("^condition_", "", code))
}

# Full edge standardization with consistent ordering
process_pairs <- function(df) {
  df %>%
    mutate(
      code1 = as.numeric(sub("^condition_", "", Disease1)),
      code2 = as.numeric(sub("^condition_", "", Disease2)),
      code_left  = pmin(code1, code2),
      code_right = pmax(code1, code2),
...
ggsave("./figures/Fig_2d.png", width = 8, height = 6, dpi = 600)
```
...
```{r}
# Compute senior-specific edges
senior_specific_edges <- setdiff(
  senior_edges,
  union(mature_edges, young_edges)
)

# Output summary
cat("Number of edges unique to senior group (vs mature + young adult):", 
    length(senior_specific_edges), "\n")

# Optional: print the edges
print(senior_specific_edges)

# Compute mature adult-specific edges
mature_specific_edges <- setdiff(
  mature_edges,
  union(senior_edges, young_edges)
)

# Output summary
cat("Number of edges unique to mature adult group (vs senior + young adult):", 
    length(mature_specific_edges), "\n")

# Optional: print the edges
print(mature_specific_edges)

# Compute young adult-specific edges
young_specific_edges <- setdiff(
  young_edges,
  union(senior_edges, mature_edges)
)

# Output summary
cat("Number of edges unique to young adult group (vs senior + mature adult):", 
    length(young_specific_edges), "\n")

# Optional: print the edges
print(young_specific_edges)

# Helper function to extract unique node IDs from edge strings
extract_nodes <- function(edge_vector) {
  unique(unlist(strsplit(edge_vector, "-")))
}

# Get unique nodes from each edge group
...
# Output
cat("Unique nodes in senior-specific edges:\n")
print(senior_unique_nodes)

cat("\nUnique nodes in mature adult-specific edges:\n")
print(mature_unique_nodes)

cat("\nUnique nodes in young adult-specific edges:\n")
print(young_unique_nodes)

```
```{r}
# Lookup table with names and categories
lookup_table <- frequency %>%
...
# Build senior-specific edge table
senior_edge_table <- tibble(edge = senior_specific_edges) %>%
  ...
  mutate(
    ...
    in_unstratified = ifelse(paste(code1, code2, sep = "-") %in% unstrat_edges |
                             paste(code2, code1, sep = "-") %in% unstrat_edges,
                             "Yes", "No")
  ) %>%
  select(disease_1, category_1, disease_2, category_2, in_unstratified)

# Preview and save
print(senior_edge_table)
write.csv(senior_edge_table, "./additional stats/senior_specific_edges_with_categories.csv", row.names = FALSE)

# Lookup table with names and categories
lookup_table <- frequency %>%
  select(Code.from.DAP.data, Disease.Name, Disease.Category) %>%
  mutate(Code.from.DAP.data = as.character(Code.from.DAP.data))
...
# Reuse the same lookup_table and functions defined above

# Build mature-specific edge table
mature_edge_table <- tibble(edge = mature_specific_edges) %>%
  separate(edge, into = c("code1", "code2"), sep = "-", convert = TRUE) %>%
  mutate(
    disease_1 = sapply(code1, lookup_disease_name),
    category_1 = sapply(code1, lookup_disease_category),
