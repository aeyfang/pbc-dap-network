...
```{r}
# Calculate p-values and determine significance with BH correction
results <- direct_expected_counts %>%
  ...
  mutate(
    adj_p_value_1to2 = p.adjust(p_value_1to2, method = "BH"),
    adj_p_value_2to1 = p.adjust(p_value_2to1, method = "BH"),
...
write_xlsx(direct_significant_pairs, "./network edges/direct_significant_pairs_01.xlsx")

# Create the disease lookup table
disease_lookup <- disease_frequencies %>%
  dplyr::select(Code = Numerical.Codes, Disease.Name) %>%
  mutate(Code = as.numeric(Code))  # Convert Code to numeric

# Add disease names to the Cytoscape edges and include both p-values
lookup_table <- direct_significant_pairs %>%
  ...
  # Include both raw and adjusted p-values
  dplyr::select(source, source_name, target, target_name, p_value, adjusted_p_value)

# Save the lookup table with both p-values
write.csv(lookup_table, "./lookup tables/lookup_directed_01.csv", row.names = FALSE)
...
write.csv(comprehensive_results, 
          "./CSV network edges/all_directed_edges_01.csv", 
          row.names = FALSE)
```
