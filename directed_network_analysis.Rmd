...
undirected_comorbid_pairs <- read.csv("./network tables/formatted_pairs_unstrat.csv")
...
analyze_disease_pair <- function(disease1, disease2, 
  ...
  temporal_data1 <- all_conditions %>%
    ...
    dplyr::select(dog_id, hs_diagnosis_year, hs_diagnosis_month) %>%
    dplyr::mutate(year1 = hs_diagnosis_year, month1 = hs_diagnosis_month)
  
  temporal_data2 <- all_conditions %>%
    dplyr::filter(dog_id %in% dogs_with_both,
           numerical_code == disease2) %>%
    dplyr::select(dog_id, hs_diagnosis_year, hs_diagnosis_month) %>%
    dplyr::mutate(year2 = hs_diagnosis_year, month2 = hs_diagnosis_month)
...
direct_expected_counts <- suppressWarnings({
  future_map_dfr(1:nrow(undirected_comorbid_pairs), ~{
...
write.csv(
  ...
  sprintf("./network tables/formatted_pairs_directed_%dm.csv", window_size)
)
```
