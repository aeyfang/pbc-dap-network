...
write.csv(as_tibble(table1), "./summary statistics/demographics.csv", row.names = FALSE)
```
...
if(file.exists("./directed network/diagnosis_date_data.csv")) {
  
  check_data <- read.csv("./directed network/diagnosis_date_data.csv")
  ...
  ggsave("./supplementary figures/S3_Fig.tiff", medical_history_boxplot, 
         width = 9, height = 6, dpi = 600, compression = "lzw")
  
} else {
  cat("Medical history data not found. Run data_directed.Rmd first.\n")
}
```
