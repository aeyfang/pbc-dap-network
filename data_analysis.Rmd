---
title: "data_analysis"
author: "Antoinette Fang"
date: "2024-07-21"
output: html_document
---


```{r load packages}
library(tidyverse)
library(caret)
library(writexl)
library(dplyr)
library(furrr)
library(Matrix)
```

```{r setup}
# import data 
data<-read.csv("./clean data/cleaned_all.csv")
disease_frequencies<-read.csv("./frequency tables/disease_frequencies_all.csv")

# Sort data by a unique identifier
data <- data %>% arrange(dog_id)  # Assuming dog_id is a unique identifier

# Separate predictors
predictors <- data %>%
  dplyr::select(dd_age_years, dd_weight_lbs, dd_breed_pure_or_mixed,
         recode.sex_2, recode.sex_3, recode.sex_4)

# Separate response variables
responses <- data %>%
  dplyr::select(starts_with("condition_"), starts_with("hs_cancer_types"))
```

```{r helper functions}
# Function to get numerical code for cancer types
get_cancer_code <- function(cancer_type) {
  if (is.character(cancer_type) && startsWith(cancer_type, "hs_cancer_types_")) {
    cancer_name <- sub("hs_cancer_types_", "", cancer_type)
    code <- disease_frequencies %>%
      filter(Disease.Name == cancer_name) %>%
      pull(Numerical.Codes)
    return(if(length(code) == 0) cancer_type else as.numeric(code))
  }
  return(cancer_type)
}

# Function to clean disease names
clean_disease_name <- function(name) {
  if(startsWith(name, "condition_")) {
    return(sub("^condition_", "", name))
  } else if(startsWith(name, "hs_cancer_types_")) {
    return(get_cancer_code(name))
  }
  return(name)
}

# Create standardized disease lookup table
disease_lookup <- disease_frequencies %>%
  mutate(
    condition_code = paste0("condition_", Numerical.Codes),
    cancer_code = case_when(
      Disease.Category == "Cancer" ~ paste0("hs_cancer_types_", tolower(gsub(" ", "_", Disease.Name))),
      TRUE ~ NA_character_
    ),
    code = coalesce(cancer_code, condition_code)
  ) %>%
  dplyr::select(code, Disease.Name)
```

```{r LRM for each disease}
# Function to fit logistic regression and return coefficients and p-values
fit_logistic <- function(response, predictors) {
  model <- glm(response ~ ., data = predictors, family = binomial(link = "logit"))
  coef_summary <- summary(model)$coefficients
  result <- data.frame(
    Term = rownames(coef_summary),
    Coefficient = coef_summary[, "Estimate"],
    P_value = coef_summary[, "Pr(>|z|)"]
  )
  return(result)
}

# Fit models and combine results
model_results <- map(responses, ~fit_logistic(.x, predictors))
coef_pvalue_df <- do.call(rbind, lapply(names(model_results), function(disease) {
  df <- model_results[[disease]]
  df$Disease <- disease
  return(df)
}))

# Create wide format dataframe with coefficients and p-values
coef_pvalue_wide <- coef_pvalue_df %>%
  pivot_wider(
    id_cols = Disease,
    names_from = Term,
    values_from = c(Coefficient, P_value),
    names_glue = "{Term}_{.value}"
  )

# Reorder columns
col_order <- c("Disease",
               paste0("(Intercept)_", c("Coefficient", "P_value")),
               paste0(colnames(predictors), "_Coefficient"),
               paste0(colnames(predictors), "_P_value"))
coef_pvalue_wide <- coef_pvalue_wide[, col_order]

# Add disease names and save results
coef_pvalue_wide_named <- coef_pvalue_wide %>%
  left_join(disease_lookup, by = c("Disease" = "code")) %>%
  relocate(Disease.Name, .after = Disease)

write.csv(coef_pvalue_wide_named, 
          "./undirected network/disease_coefficients_and_pvalues_all.csv", 
          row.names = FALSE)

# Extract coefficients for further analysis
coef_df <- coef_pvalue_wide %>%
  dplyr::select(Disease, ends_with("_Coefficient")) %>%
  rename_with(~str_remove(., "_Coefficient"), ends_with("_Coefficient"))

coef_df_named <- coef_df %>%
  left_join(disease_lookup, by = c("Disease" = "code")) %>%
  relocate(Disease.Name, .after = Disease)

write.csv(coef_df_named, 
          "./undirected network/disease_coefficients_only_all.csv", 
          row.names = FALSE)
```

```{r calculate individual probabilities}
# Function to calculate probability
calculate_probability <- function(predictors, coefficients) {
  predictors_with_intercept <- cbind(1, predictors)
  linear_predictor <- as.matrix(predictors_with_intercept) %*% coefficients
  prob <- 1 / (1 + exp(-linear_predictor))
  return(prob)
}

# Calculate risks for all diseases
risk_scores <- map(1:nrow(coef_df), function(i) {
  disease_coeffs <- as.numeric(coef_df[i, -1])
  calculate_probability(predictors, disease_coeffs)
})

# Combine results into dataframe
risk_scores_df <- as.data.frame(do.call(cbind, risk_scores))
colnames(risk_scores_df) <- coef_df$Disease

# Normalize probabilities
normalize_probabilities <- function(risk_scores_df, data) {
  normalized_scores <- map2_df(risk_scores_df, names(risk_scores_df), function(probabilities, disease) {
    actual_count <- sum(data[[disease]])
    total_prob <- sum(probabilities)
    scaling_factor <- actual_count / total_prob
    return(probabilities * scaling_factor)
  })
  names(normalized_scores) <- names(risk_scores_df)
  return(normalized_scores)
}

risk_scores_df <- normalize_probabilities(risk_scores_df, responses)
```

```{r calculate variance}
# Function to create stratified folds and fit models
create_stratified_folds_and_fit_models <- function(response, predictors, k = 6) {
  data <- cbind(response, predictors)
  
  # Separate positive and negative cases
  positive_cases <- data[response == 1, ]
  negative_cases <- data[response == 0, ]
  
  # Create balanced folds
  create_balanced_folds <- function(cases, k) {
    n <- nrow(cases)
    fold_size <- floor(n / k)
    folds <- rep(1:k, each = fold_size, length.out = n)
    set.seed(42)
    folds <- sample(folds)
    split(seq_len(n), folds)
  }
  
  positive_folds <- create_balanced_folds(positive_cases, k)
  negative_folds <- create_balanced_folds(negative_cases, k)
  combined_folds <- map2(positive_folds, negative_folds, c)
  
  # Fit models for each fold
  models <- map(combined_folds, function(fold) {
    fold_data <- data[fold, ]
    model <- glm(response ~ ., data = fold_data, family = binomial(link = "logit"))
    return(coef(model))
  })
  
  return(models)
}

# Create stratified folds and fit models
stratified_models <- map2(
  responses,   
  rep(list(predictors), ncol(responses)),
  create_stratified_folds_and_fit_models
)

# Calculate probabilities for all folds
calculate_all_probabilities <- function(fold_coefs, full_predictors) {
  map(fold_coefs, ~calculate_probability(full_predictors, .x))
}

all_probabilities <- future_map(
  stratified_models,
  ~calculate_all_probabilities(.x, predictors),
  .options = furrr_options(seed = TRUE)
)

# Calculate mean probabilities and variance
mean_probabilities <- map(all_probabilities, ~Reduce(`+`, .x) / length(.x))

calculate_variance <- function(probs, mean_prob) {
  variance <- Reduce(`+`, map(probs, ~.x - mean_prob^2)) / (length(probs) - 1)
  return(variance)
}

unbiased_variance <- map2(all_probabilities, mean_probabilities, calculate_variance)
variance_df <- as.data.frame(do.call(cbind, unbiased_variance))
colnames(variance_df) <- colnames(responses)

write.csv(variance_df, 
          "./undirected network/personalized_probability_with_variance_all.csv", row.names = FALSE)
```

```{r identify comorbid pairs}
# Function to calculate pair statistics
calculate_pair_statistics <- function(pair) {
  disease1 <- pair[1]
  disease2 <- pair[2]
  
  test_value <- sum(data[[disease1]] & data[[disease2]])
  
  risk1 <- risk_scores_df[[disease1]]
  risk2 <- risk_scores_df[[disease2]]
  var1 <- variance_df[[disease1]]
  var2 <- variance_df[[disease2]]
  
  expected_value <- sum(risk1 * risk2)
  unadjusted_var <- sum(risk1 * risk2 * (1 - risk1 * risk2))
  adjusted_var <- sum(var1 * risk2^2 + var2 * risk1^2 + var1 * var2)
  final_var <- unadjusted_var + adjusted_var
  
  z_score <- (test_value - expected_value) / sqrt(final_var)
  p_value <- 1 - pnorm(z_score)
  
  c(Disease1 = disease1,
    Disease2 = disease2,
    TestValue = test_value, 
    ExpectedValue = expected_value, 
    UnadjustedVariance = unadjusted_var,
    AdjustedVariance = adjusted_var, 
    FinalVariance = final_var, 
    PValue = p_value)
}

# Generate all pairs and calculate statistics
network_attributes <- combn(names(responses), 2, calculate_pair_statistics, simplify = FALSE)

# Create network dataframe
network_df <- do.call(rbind, network_attributes) %>% 
  as.data.frame() %>%
  mutate(across(TestValue:PValue, as.numeric))

# Apply multiple testing correction
network_df$AdjustedPValue <- p.adjust(network_df$PValue, method = "BH")

# Save all pairs with variances
write.csv(network_df, "./summary statistics/all_pairs_variances.csv")

# Filter significant pairs
significant_pairs <- network_df %>% 
  filter(AdjustedPValue < 0.01) %>%
  arrange(AdjustedPValue)

# Format significant pairs for Cytoscape
formatted_significant_pairs <- significant_pairs %>%
  mutate(
    Disease1 = sapply(Disease1, clean_disease_name),
    Disease2 = sapply(Disease2, clean_disease_name)
  ) %>%
  dplyr::select(Disease1, Disease2)

# Save results
write.csv(significant_pairs, 
          "./CSV network edges/significant_pairs_all.csv", 
          row.names = FALSE)
write_xlsx(formatted_significant_pairs, 
           "./network edges/formatted_pairs_all.xlsx")
```


```{r reference tables}
# Create final reference table with p-values
lookup_table <- formatted_significant_pairs %>%
  mutate(
    Disease1 = as.character(Disease1),
    Disease2 = as.character(Disease2)
  ) %>%
  # Create clean disease lookup to match our codes
  left_join(
    disease_frequencies %>%
      mutate(code = as.character(Numerical.Codes)) %>%
      select(code, Disease.Name),
    by = c("Disease1" = "code")
  ) %>%
  rename(Disease1_name = Disease.Name) %>%
  left_join(
    disease_frequencies %>%
      mutate(code = as.character(Numerical.Codes)) %>%
      select(code, Disease.Name),
    by = c("Disease2" = "code")
  ) %>%
  rename(Disease2_name = Disease.Name) %>%
  select(Disease1, Disease1_name, Disease2, Disease2_name) %>%
  cbind(pval = significant_pairs$AdjustedPValue)

write.csv(lookup_table, 
          "./lookup tables/lookup_all.csv", 
          row.names = FALSE)

# Create comprehensive table with all statistics
comprehensive_table <- network_df %>%
  mutate(
    Disease1_key = sapply(Disease1, clean_disease_name),
    Disease2_key = sapply(Disease2, clean_disease_name)
  ) %>%
  # Join with disease frequencies for names
  left_join(
    disease_frequencies %>%
      mutate(code = as.character(Numerical.Codes)) %>%
      select(code, Disease.Name),
    by = c("Disease1_key" = "code")
  ) %>%
  rename(Disease1_name = Disease.Name) %>%
  left_join(
    disease_frequencies %>%
      mutate(code = as.character(Numerical.Codes)) %>%
      select(code, Disease.Name),
    by = c("Disease2_key" = "code")
  ) %>%
  rename(Disease2_name = Disease.Name) %>%
  select(
    Disease1, Disease1_key, Disease1_name,
    Disease2, Disease2_key, Disease2_name,
    TestValue, ExpectedValue, 
    UnadjustedVariance, AdjustedVariance, FinalVariance,
    PValue, AdjustedPValue
  ) %>%
  arrange(AdjustedPValue)

write.csv(comprehensive_table, 
          "./CSV network edges/comprehensive_disease_pairs_all.csv", 
          row.names = FALSE)
```

```{r}
# Focus on specific diseases
diseases_of_interest <- c(
  "condition_908",  # Chronic kidney disease (CKD)
  "condition_1307", # Diabetes mellitus
  "condition_514"   # Hypertension
)

# Extract relevant predictors and responses
predictors <- data %>%
  dplyr::select(dd_age_years, dd_weight_lbs, dd_breed_pure_or_mixed,
                recode.sex_2, recode.sex_3, recode.sex_4)

responses <- data %>%
  dplyr::select(all_of(diseases_of_interest))

# Add intercept term to predictors
X <- as.matrix(cbind(1, predictors))

# Function to analyze linear predictor distribution
analyze_linear_predictor <- function(disease_name, response, X) {
  # Fit logistic regression
  model <- glm(response ~ X - 1, family = binomial(link = "logit"))
  
  # Calculate linear predictor for all individuals
  linear_pred <- X %*% coef(model)
  
  # Get summary statistics
  summary_stats <- c(
    min = min(linear_pred),
    q1 = quantile(linear_pred, 0.25),
    median = median(linear_pred),
    mean = mean(linear_pred),
    q3 = quantile(linear_pred, 0.75),
    max = max(linear_pred),
    range = diff(range(linear_pred))
  )
  
  # Calculate probability ranges
  probs <- 1/(1 + exp(-linear_pred))
  prob_stats <- c(
    min_prob = min(probs),
    max_prob = max(probs)
  )
  
  list(
    disease = disease_name,
    linear_pred_stats = summary_stats,
    prob_stats = prob_stats,
    linear_pred = linear_pred
  )
}

# Analyze each disease
results <- map(diseases_of_interest, function(disease) {
  analyze_linear_predictor(disease, responses[[disease]], X)
})

# Print results
disease_names <- c(
  "condition_908" = "Chronic Kidney Disease",
  "condition_1307" = "Diabetes Mellitus", 
  "condition_514" = "Hypertension"
)

for(i in seq_along(results)) {
  cat("\nResults for", disease_names[diseases_of_interest[i]], ":\n")
  cat("\nLinear Predictor (betahat' x_i) Statistics:\n")
  print(round(results[[i]]$linear_pred_stats, 4))
  cat("\nProbability Range:\n")
  print(round(results[[i]]$prob_stats, 4))
  
  # Create histogram of linear predictors
  hist_data <- data.frame(linear_pred = results[[i]]$linear_pred)
  print(
    ggplot(hist_data, aes(x = linear_pred)) +
      geom_histogram(bins = 50, fill = "blue", alpha = 0.5) +
      ggtitle(paste("Distribution of Linear Predictors for", 
                    disease_names[diseases_of_interest[i]])) +
      xlab("Linear Predictor (betahat' x_i)") +
      ylab("Count") +
      theme_minimal()
  )
}
```

