...
health_condition_wide <- health_condition_wide %>%
  mutate(
    condition_1210 = if_else(
      (if("condition_1207" %in% names(.)) condition_1210 else 0) == 1 |
      (if("condition_1108" %in% names(.)) condition_1111 else 0) == 1,
      1, 0
    )
  ) %>%
...
health_condition_wide <- health_condition_wide %>%
  mutate(
    condition_1804 = if_else(
      rowSums(select(., starts_with("condition_180")), na.rm = TRUE) > 0,
      1, 0
    )
  ) %>%
  dplyr::select(-starts_with("condition_180"), condition_1804)
...
generate_frequency_table <- function(data, suffix = "") {
  ...
  # Update condition name for IVDD code 1207
  disease_frequencies$Disease.Name[disease_frequencies$Code.from.DAP.data == "1207"] <- "Intervertebral disk disease (IVDD)"
